...
const patchRemainNodes = (schema, oldChildren, newChildren) => {
  ...
  while (oldChildLen - left - right > 0 && newChildLen - left - right > 0) {
    ...
    if (updateLeft) {
      ...
      // For structural nodes (like headings), we want to preserve the structure
      // but mark the content as changed
      if (
        !isTextNode(leftOldNode) &&
        !Array.isArray(leftOldNode) &&
        !isTextNode(leftNewNode) &&
        !Array.isArray(leftNewNode) &&
        matchNodeType(leftOldNode, leftNewNode)
      ) {
        // Same node type but different content - do a recursive diff
        finalLeftChildren.push(
          patchDocumentNode(schema, leftOldNode, leftNewNode),
        );
        left += 1;
...
export const patchTextNodes = (schema, oldNode, newNode) => {
  ...
  // Extract text and marks from nodes
  const oldTextData = oldNode.map((n) => ({
    text: getNodeText(n),
    marks: getNodeMarks(n),
  }));
  const newTextData = newNode.map((n) => ({
    text: getNodeText(n),
    marks: getNodeMarks(n),
  }));

  const oldText = oldTextData.map((data) => data.text).join('');
  const newText = newTextData.map((data) => data.text).join('');

  // For simple cases where we can preserve formatting, try to match marks
  if (oldTextData.length === 1 && newTextData.length === 1) {
    const oldMarks = oldTextData[0].marks;
    const newMarks = newTextData[0].marks;
    ...
    // Map diffs to nodes with preserved marks
        ...
        const baseMarks =
          type === DiffType.Deleted
            ...
            : type === DiffType.Inserted
              ? newMarks
              : oldMarks; // Use old marks for unchanged text

        const diffMark =
          type !== DiffType.Unchanged ? createDiffMark(schema, type) : null;
        const allMarks = diffMark ? [...baseMarks, diffMark] : baseMarks;

        return createTextNode(schema, sentence, allMarks);
      });
    });
...
export const computeChildEqualityFactor = (node1, node2) => {
  ...
  let equalityScore = 0.5; // Base score for same node type
  ...
  if (allAttrs.length > 0) {
    const matchingAttrs = allAttrs.filter(
      (attr) => attrs1[attr] === attrs2[attr],
    ).length;
    equalityScore += (matchingAttrs / allAttrs.length) * 0.3;
  }

  // Check content similarity for text nodes
  ...
  if (children1.length === children2.length && children1.length > 0) {
    equalityScore += 0.2;
  }

  return Math.min(1, equalityScore);
};
...
export const createDiffNode = (schema, node, type) => {
  return mapDocumentNode(node, (currentNode) => {
    ...
    // For non-text nodes, we need to add a diff mark wrapper around their content
    // but preserve the node structure
    return currentNode;
  });
};
...
function mapDocumentNode(node, mapper) {
  ...
  const mappedContent = node.content?.content
    ? node.content.content.map((childNode) =>
        mapDocumentNode(childNode, mapper),
      )
    : [];

  const copy = node.copy(Fragment.from(mappedContent.filter((n) => n)));
  const result = mapper(copy);
  return result !== undefined ? result : copy;
}
